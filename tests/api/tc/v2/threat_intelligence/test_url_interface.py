...
class TestUrlIndicators(TestThreatIntelligence):
    ...
    def tests_ti_url_create(self):
        """Create an indicator using specific interface."""
        indicator_data = {
            self.indicator_field_custom: self.ti_helper.rand_url(),
            'confidence': randint(0, 100),
            'owner': self.owner,
            'rating': randint(0, 5),
        }
        ti = self.ti.url(**indicator_data)
        r = ti.create()
