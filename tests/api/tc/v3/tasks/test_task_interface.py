...
class TestTasks(TestV3):
    ...
    def test_task_create_and_retrieve_nested_types(self, request: FixtureRequest):
        ...
        assert task.model.notes.data is not None, 'No notes were returned.'
        ...
        # is only available due to params being added to the get() method.
        assert task.model.artifacts.data is not None, 'No artifacts were returned.'
    ...
    def test_task_all_filters(self, request: FixtureRequest):
        ...
        if task.model.status is not None:
            tasks.filter.status(TqlOperator.EQ, task.model.status)
        tasks.filter.workflow_phase(TqlOperator.EQ, task_data['workflow_phase'])
        tasks.filter.workflow_step(TqlOperator.EQ, task_data['workflow_step'])
        tasks.filter.xid(TqlOperator.EQ, task_data['xid'])
        tasks.filter.required(TqlOperator.EQ, False)
        tasks.filter.name(TqlOperator.EQ, task_data['name'])
    ...
    def test_task_get_many(self, request: FixtureRequest):
        """Test Task Get Many"""
        # [Pre-Requisite] - create case
        case = self.v3_helper.create_case()
        if case.model.id is None:
    ...
    def test_task_get_single_by_id_properties(self, request: FixtureRequest):
        ...
        for _ in range(0, note_count):
            ...
            note_data['data'].append({'text': text})
        ...
        task.model.artifacts = task_data['artifacts']
        task.model.assignee = task_data['assignee']
        task.model.case_id = task_data['case_id']
        task.model.dependent_on_id = task_data['dependent_on_id']
        task.model.description = task_data['description']
        task.model.due_date = task_data['due_date']
        task.model.completed_date = task_data['completed_date']
        task.model.name = task_data['name']
        task.model.notes = task_data['notes']
        task.model.required = task_data['required']
        task.model.status = task_data['status']
        task.model.workflow_phase = task_data['workflow_phase']
        ...
        assert task.model.name == task_data['name']
        assert task.model.required == task_data['required']
        assert task.model.status == task_data['status']
        assert task.model.workflow_phase == task_data['workflow_phase']
        assert task.model.assignee.type == assignee['type']
        # TODO: revisit this assertion
        # assert task.model.assignee.data.user_name == assignee['data']['userName']
        assert task.model.notes.data is not None, 'Notes should not be None'
        ...
        assert task.model.artifacts.data is not None, 'Artifacts should not be None'
