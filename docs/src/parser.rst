...
Batch Arguments
----------------
The ThreatConnect batch API endpoint requires a few arguments for setting the batch job.  These arguments have sane default values and are not often required to be added to the ``install.json`` configureation file.  These default values can also be overriden with methods provided in the :py:mod:`~tcex.tcex_job.TcExJob` module.
