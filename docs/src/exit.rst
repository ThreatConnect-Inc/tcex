...
Setting Exit Code
------------------
Some failures do not warrant an immediate exit.  In such case the :py:meth:`~tcex.tcex.TcEx.exit_code` method allows an exit code to be set for when the :py:meth:`~tcex.tcex.TcEx.exit` method is called.  This allows the app to continue execution and still notify the ThreatConnect Platform that a failure occured.
