...
class RegistryKey(Indicator):
    ...
    def __init__(self, tcex, key_name, value_name, value_type, owner=None, **kwargs):
        ...
        """
        super().__init__(tcex, 'Registry Key', 'registryKey', 'registryKeys', owner, **kwargs)
    ...
    def can_create(self):
        """Return True if registry key can be created.

        If the key_name, value_name, and value_type has been provided returns that the Registry
        Key can be created, otherwise returns that the Registry Key cannot be created.
