...
class TcexJson:
    ...
    def replace_env_variables(self, json_data: dict) -> dict:
    ...
    def update_package_app_name(self, json_data):
        ...
        if self.package_app_name in [None, '', 'TC_-_', 'TCPB_-_', 'TCVC_-_', 'TCVW_-_']:
            ...
            app_name = (
                os.path.basename(os.getcwd()).lower().replace(self.ij.data.app_prefix.lower(), '')
            )
            ...
            app_name = f'{self.ij.data.app_prefix}{app_name}'
    ...
    @staticmethod
    def update_variable_pattern_env(json_data: dict) -> dict:
        """Update all environment variables to latest pattern.
