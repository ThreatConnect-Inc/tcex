...
class Permutations:
    ...
    def _gen_permutations(self, index: Optional[int] = 0, args: Optional[list] = None):
        ...
        Args:
            ...
            args: Defaults to None. The current list of args.
        """
        if args is None:
            args = []
        try:
            ...
            if self.ij.data.runtime_level.lower() in [
                ...
                display = self.lj.get_param(name).display
                hidden = self.lj.get_param(name).hidden
            else:
                name = self.ij.data.param_names[index]
                display = False

            param: Optional[ParamsModel] = self.ij.data.get_param(name)
            if param.type is None:
                self.handle_error(f'No param type found in install.json for "{name}".', halt=True)

            if (
                self.ij.data.runtime_level.lower() == 'organization'
                ...
                if param.type.lower() == 'boolean':
                    for val in [True, False]:
                        ...
                        args.pop()
                elif param.type.lower() == 'choice':
                    valid_values = self.ij.expand_valid_values(param.valid_values)
    ...
    def init_permutations(self):
        ...
        if self._input_permutations is None and self._output_permutations is None:
            ...
            self.db_create_table(self.input_table, self.ij.data.param_names)
            self.db_insert_record(self.input_table, self.ij.data.params_names)
    ...
    def outputs_by_inputs(self, inputs):
        ...
        # iterate of InstallJsonModel -> PlaybookModel -> OutputVariablesModel
        for o in self.ij.data.playbook.output_variables:
            if self.lj.outputs_dict.get(o.name) is None:
                # an output not listed in layout.json should always be shown
                valid = True
            else:
                # all other outputs must be validated
                display = self.lj.outputs_dict.get(o.name, {}).get('display')
    ...
    def permutations(self):
        ...
        # create db for permutations testing
        self.db_create_table(self.input_table, self.ij.data.params_names)
        self.db_insert_record(self.input_table, self.ij.data.params_names)
    ...
    def validate_input_variable(self, input_name, inputs):
        ...
        table = f'temp_{random.randint(100,999)}'  # nosec
        self.db_create_table(table, self.ij.data.params_names)
        self.db_insert_record(table, self.ij.data.params_names)
