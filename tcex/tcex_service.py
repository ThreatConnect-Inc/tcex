...
class TcExService(object):
    ...
    def format_request_headers(headers):
        ...
        # headers_ = []
        # for h in headers:
        #     headers_.append((h.get('name'), h.get('value')))
        # return headers_
        headers_ = {}
        for h in headers:
    ...
    def fire_event(self, callback, **kwargs):
        ...
        for trigger_id, config in self.configs.items():
            ...
            if trigger_type == 'custom':
                ...
                webhook_trigger_id = kwargs.get('trigger_id')
                if trigger_id != webhook_trigger_id:
                    # only proceed if webhook is for current config
                    continue
