...
class IndicatorAttributeFilter(FilterABC):
    ...
    def default(self, operator: Enum, default: bool):
        ...
        Args:
            operator: The operator enum for the filter.
            default: A flag to include an attribute in indicator associations.
        """
    ...
    def indicator_id(self, operator: Enum, indicator_id: int | list):
        ...
        """
        if isinstance(indicator_id, list) and operator not in self.list_types:
            raise RuntimeError(
                'Operator must be CONTAINS, NOT_CONTAINS, IN'
                'or NOT_IN when filtering on a list of values.'
            )
