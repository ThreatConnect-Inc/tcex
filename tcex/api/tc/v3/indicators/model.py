...
class IndicatorModel(
    ...
    active_locked: bool = Field(
        None,
        description='Lock the inidcator active value?.',
        methods=['POST', 'PUT'],
        read_only=False,
        title='activeLocked',
    )
    ...
    false_positive_last_reported: Optional[datetime] = Field(
        None,
        description='The date and time of the last false postitive reported for this indicator.',
        methods=['POST', 'PUT'],
        read_only=False,
        title='falsePositiveLastReported',
    )
    file_action: Optional['FileActionModel'] = Field(
        None,
    ...
    file_occurrence: Optional['FileOccurrencesModel'] = Field(
        None,
        description='A list of file occurrences associated with this indicator.',
        methods=['POST', 'PUT'],
        read_only=False,
        title='fileOccurrence',
    )
    ...
    whois_active: Optional[str] = Field(
        None,
    ...
    @validator('file_action', always=True)
    def _validate_file_action(cls, v):
        if not v:
            return FileActionModel()
        return v

    @validator('file_occurrence', always=True)
    def _validate_file_occurrence(cls, v):
        if not v:
...
from tcex.api.tc.v3.file_actions.model import FileActionModel
from tcex.api.tc.v3.file_occurrences.model import FileOccurrencesModel
from tcex.api.tc.v3.groups.model import GroupsModel
